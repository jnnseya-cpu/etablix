...
export const DPS_SEED = [
  ...
  },
  {
    ...
    notes: "Self-service: no introduction email, apply through the Supplier Registration Service. Read first: READ FIRST RM6242 DPS Needs v4.0; Attachment 6 is the DPSQ (the selection questionnaire) itself, so our requirement list here can be replaced with fact; Attachment 1 Filter Matrix sets the elements to select; Attachment 3 Financial Assessment Template is the likely pinch point for a young company — complete it early. Restricted procedure above threshold: any clarification goes through the portal message board, never direct email, so the answer is published to all applicants. Term extended to 23/02/2029. Confirmed by CCS Commercial Operations in writing (Sept 2026): applicants select both the services they can provide AND the contract value bands they can financially support, and the route in is simply to complete the supplier questionnaires including the Financial Viability Risk Assessment. The finance team assesses what is submitted and comes back if it needs a guarantor or alternative financial evidence — so a guarantor is NOT a precondition of applying here and chasing one before submitting delays the application for nothing. The value band is the lever: the financial assessment is proportionate to the band claimed, so a company with no filed accounts selects the lowest band it can genuinely support and is uplifted later, rather than failing an assessment against a band it chose optimistically. A DPS stays open, so a band can be amended.",
  },
  {
