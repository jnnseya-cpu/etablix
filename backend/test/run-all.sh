...
echo "=== unit ==="
for t in clientflow.test workingdays.test pricing.test tenderpack.test seo.test editorial.test reach.test deletion.test bidcheck.test controlcheck.test mock.test indexnow.test deploy.test mail.test pipeline.test concurrent.test store-kill.test; do
  printf '  %-22s ' "$t"
...
echo "=== end to end ==="
SUITES="money.e2e clientflow.e2e enquiry-to-engagement.e2e portal-promises.e2e upload-dedupe.test pipeline.e2e bid.e2e control.e2e retention.e2e circle.e2e security.e2e seo.e2e"
