...
/**
 * `prompt` is the WHOLE prompt, not only the task.
 *
 * One reply depends on it: the challenger's findings register. A bid with an
 * unanswered mandatory requirement and a fitness-for-purpose sentence in it
 * has two critical findings, correctly, and no gate should ever release it.
 * A bid where those have been corrected has neither. Keying the reply on the
 * task alone would make the corrected case unreachable, and the only way to
 * test the release path would be an override — which is exactly the bypass
 * that makes a gate a suggestion.
 */
export function answer(task, prompt = "") {
  ...
  if (/Every lens is written\. Three things remain/.test(task) && /THE BID HAS BEEN CORRECTED/.test(prompt))
    return "## 0 · The challenge in one paragraph\nThe two critical findings from the previous challenge have been closed: SUB-4 is now answered and the fitness-for-purpose sentence has been replaced with reasonable skill and care. What remains is material but not disqualifying, and the submission can go against recorded dispositions.\n\n## 10 · The findings register\n| Ref | Lens | Severity | Where | The finding | What must happen | Disposition |\n|---|---|---|---|---|---|---|\n| CH-1 | compliance | LOW | Section 6, first paragraph | Insurance evidence sits inside section 6 rather than as a separate attachment | Move it to a separate attachment named as the invitation requires | Accepted by J Nseya on 2026-09-09 |\n| CH-2 | commercial | HIGH | Section 4 against the resource schedule | Eighteen welfare units committed, sixteen priced | Price two further units or amend the commitment to sixteen | Accepted by J Nseya on 2026-09-09, priced in |\n| CH-3 | evaluator | MEDIUM | Method statement paragraph 4 | The shift commitment and its resource evidence are two sections apart | Repeat the resource figures in the method statement | Accepted by J Nseya on 2026-09-09 |\n\nCritical 0, high 1, medium 1, low 1. Nothing hard-blocks this submission; the high finding carries an authorised disposition.\n\n## 11 · The challenge certificate\n**Independence.** The document under review was produced by run bid-0091; this challenge is run bid-0112 on a separate prompt lineage and a different model route.\n\n**What was examined.** The corrected submission against the invitation, the weightings, the price, the qualifications and the evidence schedule.\n\n**The most consequential finding.** CH-2: two welfare units committed and not priced.\n\n**Whether this submission can go.** CONDITIONAL.\n\n**What must happen before it goes.** Nothing that blocks; the high finding is disposed of and recorded.\n\n**What this challenge could not test.** The technical lens had no design information beyond the programme.\n\n## A · Every finding traced to where it was found\n| Ref | Found in |\n|---|---|\n| CH-1 | The invitation's deliverables list |\n| CH-2 | Section 4 against the resource schedule |";
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", async () => {
    ...
    let text = answer(String(task), promptText);
