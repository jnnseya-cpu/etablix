...
/** Fetch from the platform when connected; otherwise (or on failure) fall back. */
async function fromPlatform(path, fallback) {
  if (!isConnected("veryx")) return { source: WORKSPACE, value: fallback() };
  try {
    const body = await platformFetch("veryx", path);
    return { source: LIVE, value: body.data };
  } catch (err) {
...
router.post("/agents/:type/run", async (req, res) => {
