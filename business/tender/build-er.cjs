...
[["1", "The subcontract conditions and any amendments"], ["2", "These Employer's Requirements"], ["3", "The fire strategy issued at Appendix 2"],
 ["4", "The drawings listed in the Drawings and Documents Register at section 11"], ["5", "The tenderer's accepted proposal, to the extent it does not conflict with the above"]]
...
doc.push(h2("1.3  What is NOT in this package"));
...
const rw = [2200, 2740, 2140, 1940];
doc.push(table(rw, [
  hdr(rw, ["Item", "RT-01  Standard bedroom", "RT-02  Accessible bedroom", "RT-03  Senior bedroom"]),
  ...C.rds.map((r) => r[1] === ""
    ? new TableRow({ children: [cell(r[0], { w: W, span: 4, bold: true, fill: PAPER, size: 17 })] })
    : new TableRow({ children: r.map((v, i) => cell(v, { w: rw[i], bold: i === 0, size: 16 })) }))]));

// ── 4 technical specification
doc.push(new Paragraph({ children: [new PageBreak()] }));
doc.push(h1("4  Technical specification, by element"));
S.spec.forEach(([cl, el, req]) => { doc.push(h2(cl + "  " + el)); doc.push(p(req)); });
...
[["Design calculations for the U-values, air permeability strategy and thermal bridging","Section 6"],
 ["Setting-out and base tolerance specification","4.1 and IF-01"],
 ["Maintenance access strategy drawing","4.14"],
 ["Tender programme showing every interface date at section 5","7.1"],
 ["Logistics plan for delivery, craneage and segregation","10.3"],
 ["Schedule of departures from this document, priced separately","1.1"],
...
more.push(new Paragraph({ children: [new PageBreak()] }));
more.push(h1("13  Appendices"));
[["1","Activity schedule, in the structure to be priced"],["2","Fire strategy, issued by the client's fire engineer"],
 ["3","Planning consent and conditions"],["4","Construction Environmental Management Plan"],
 ["5","Approved external colour and finish range"],["6","Bed demand curve and occupancy programme"],
 ["7","Abnormal load route assessment — draft, incomplete"],["8","Design loadings, site exposure and rainfall data"],
 ["9","Overheating criterion and the client's comfort brief"]]
